"""Schedule sources: pluggable providers of {channel: program_info} data.

Only ``SorrisiSource`` exists today. ``ScheduleSource`` is the seam a second
source would implement (see README "Cose da sapere" for why none ships yet)
without touching ``coordinator.py`` or the entities.
"""
